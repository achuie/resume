...
\begin{aSection}{Experience}
    ...
    \item Developed live controller monitoring bot in \textbf{Go}, reducing response time from \textbf{\textasciitilde 1
        day} to \textbf{\textasciitilde 10 minutes}
        \begin{subList}
        \item Created system usage statistics module to enable hardware issue support
        \item Designed and implemented module to stream controller state info from \textbf{GraphQL} over
            \textbf{websockets}
        \item Automated deployment with \textbf{GitLab} and \textbf{Kubernetes}
        \end{subList}
    \item Engineered controller system simulator for company-wide \textbf{test driven development}
        \begin{subList}
        \item Simulated \textbf{QML} UI interactions in Python to automate validation for onsite operations
        \item Emulated warehouse control systems (WCS/PLC) in \textbf{Python} for integration testing critical features
        ...
        \end{subList}
    \item Devised and programmed Industrial Task Language (\textbf{ITL}) programs to control additional robotic hardware
    \item Developed system inspection webapp using ReactJS \& Python for \textbf{forensic debugging}
    \item Architected and implemented \textbf{Pytest} framework to validate override \& migration operations on
